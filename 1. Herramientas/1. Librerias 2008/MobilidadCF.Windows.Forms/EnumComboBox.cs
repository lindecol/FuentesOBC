		...
		public Type EnumType
		{
			...
			set
			{
				...
				if ( value != null )
				{
					if ( value.IsEnum )
					{
						string[] items = Enum2.GetNames(value);
						this.Items.Clear();
		...
		public new object SelectedValue
		{
			get
			{
				if ( EnumType != null && SelectedIndex >= 0 )
					return Enum2.Parse(EnumType, SelectedItem.ToString());
				else
					return null;
			}
			set
			{
				if ( EnumType != null )
				{
					if ( Enum2.IsDefined(EnumType, value) )
						this.SelectedIndex = this.Items.IndexOf(Enum2.GetName(EnumType, value));
				}
				else
