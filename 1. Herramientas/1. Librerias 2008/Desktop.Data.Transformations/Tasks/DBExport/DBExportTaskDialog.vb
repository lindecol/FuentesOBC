...
Public Class DBExportTaskDialog

    Private m_RowGeneral As DBExportTaskDataSet.GeneralRow
    ...
    Private m_Task As DBExportTask
    Private m_dtOrigen As DataTable
    Private m_dtDestino As DataTable

    Public Shared Function Run(ByVal Task As DBExportTask) As Boolean
        Dim frm As New DBExportTaskDialog
        Dim bRes As Boolean
    ...
    Private Sub DBExportTaskDialog_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        InitData()
        If dsTask.General.Count = 0 Then
            ' Se agrega la tarea y se configuran los valores por defecto
            bsGeneral.AddNew()
            m_RowGeneral = CType(DBUtils.GetCurrentRow(bsGeneral), DBExportTaskDataSet.GeneralRow)
    ...
    Private Sub InitData()
        dsTask = CType(m_Task.dsConfig, Desktop.Data.Transformations.DBExportTaskDataSet)
    ...
    Private Sub LoadTablasDestino()
        Try
            ...
            'dsTask.General(0).TipoConexion, dsTask.General(0).CadenaConexion)

            m_Destino = GestorDatosFactory.CreateInstance(cbTipoConexion.Text, CadenaConexionTextBox.Text)
        ...
        Catch ex As Exception
            If cbTipoConexion.Text <> "" And CadenaConexionTextBox.Text <> "" Then
                ShowError(ex.Message)
            Else
                ShowError("Los campos de tipo conexion y cadena conexion no pueden estar vacios")
            End If
        Finally
            If Not m_Destino Is Nothing Then
    ...
    Private Sub LoadCamposDestino()
		...
		If dsTask.General(0).TipoConexion.ToUpper = "ODBC" Then
			If dsTask.General(0).TablaDestino.StartsWith("[") Then
				daDestino.SelectCommand.CommandText = "SELECT * FROM " & dsTask.General(0).TablaDestino
			Else
				dsTask.General(0).TablaDestino = "[" & dsTask.General(0).TablaDestino & "]"
				daDestino.SelectCommand.CommandText = "SELECT * FROM " & dsTask.General(0).TablaDestino
			End If
		Else
			daDestino.SelectCommand.CommandText = "SELECT * FROM " & dsTask.General(0).TablaDestino
    ...
    Private Sub btnGuardarMapeo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGuardarMapeo.Click
        Try
            If Not IsNullOrEmpty(GetCurrentRow(bsMapeoCampos)) Then
                Dim mapeo As DBExportTaskDataSet.MapeoCamposRow

                mapeo = CType(GetCurrentRow(bsMapeoCampos), DBExportTaskDataSet.MapeoCamposRow)
		...
		Try
			...
			If TabControl1.TabPages.Count = 1 Then
				...
				If cbTablaDestino.Text <> "" Then
					If cbTipoConexion.Text.ToUpper = "ODBC" Then
						If Not cbTablaDestino.Text.StartsWith("[") Then
							cbTablaDestino.Text = "[" + cbTablaDestino.Text + "]"
						End If
					End If
