...
#Region "Funciones Productos"
    Public Function GetProducto(ByVal Codigo As String) As ProductosDataSet.ProductosRow
        ...
        Catch ex As Exception
            Try
                Dim sql As String = "SELECT ISNULL(CodFlete,0) AS CodFlete, ISNULL(CodTipoProducto,1) AS CodTipoProducto, Codigo, Descripcion, ISNULL(GeneraMovSerial,'') AS GeneraMovSerial, Iva, Lastro, ISNULL(LeeBarras,'') AS LeeBarras, Precio, RequiereAsignacion, RequiereFlete, UnidadMedida FROM Productos WHERE (Codigo = '" + Codigo + " ')"
                Dim comando As New SqlCeCommand(sql, Connection)
                Dim adaptador As New SqlCeDataAdapter(comando)
                Dim tabla As New DataTable
                adaptador.Fill(tabla)

                Dim dtResultado As New ProductosDataSet.ProductosDataTable
                Dim resultado As ProductosDataSet.ProductosRow = dtResultado.NewProductosRow

                If (tabla.Rows.Count > 0) Then
                    resultado.CodFlete = tabla.Rows(0)("CodFlete").ToString()
                    resultado.CodTipoProducto = tabla.Rows(0)("CodTipoProducto").ToString()
                    resultado.Codigo = tabla.Rows(0)("Codigo").ToString()
                    resultado.Descripcion = tabla.Rows(0)("Descripcion").ToString()
                    resultado.GeneraMovSerial = tabla.Rows(0)("GeneraMovSerial").ToString()
                    resultado.Iva = Convert.ToDecimal(tabla.Rows(0)("Iva").ToString())
                    resultado.Lastro = tabla.Rows(0)("Lastro").ToString()
                    resultado.LeeBarras = tabla.Rows(0)("LeeBarras").ToString()
                    resultado.Precio = Convert.ToDecimal(tabla.Rows(0)("Precio"))
                    resultado.RequiereAsignacion = tabla.Rows(0)("RequiereAsignacion").ToString()
                    resultado.RequiereFlete = tabla.Rows(0)("RequiereFlete").ToString()
                    resultado.UnidadMedida = tabla.Rows(0)("UnidadMedida").ToString()
            ...
            Catch ex1 As Exception
                MovilidadCF.Logging.Logger.Write(ex1)
                Return Nothing
            End Try
        End Try
